...
#define DUMP_ADDRESSES_INTERVAL 30
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "xPay " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. xPay is probably already running."), addrBind.ToString().c_str());
        else
