...
#if defined(NDEBUG)
# error "xPay cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "xPay Signed Message:\n";
...
{
    // xPay: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // xPay: 3.5 days
static const int64 nTargetSpacing = 60; // xPay: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // xPay: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // xPay: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6f9fdda741fd885f5fadff15c7a8e8669e0c65f5f83e79a750d465b294a33fbe");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "xPay, the Rocket in the Storm that will hit the moon";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 65 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("a13b603131ba1332f23adccc8065d26bb438f0b08a29471718e9857178c4707383e40d768824b650a54ecaee9da967e3a0489add8a79fd55a28545caaaf270a0d2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520453091;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087905446;

        if (fTestNet)
        {
            block.nTime    = 1520453091;
            block.nNonce   = 385509370;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x670d55df7a37a8aa9a7d76907cf798fbba5cc8c8b3c593999398f0d5f6b3fa7e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // xPay: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("xPayMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("xPayMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static xPayMiner(CWallet *pwallet)
{
    printf("xPayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("xpay-miner");
    ...
    try { loop {
        ...
        printf("Running xPayMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&xPayMiner, pwallet));
}
